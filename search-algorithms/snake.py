...
class SearchBasedPlayer(Player):
    ...
    def bfs(self, snake: Snake, food: Food, *obstacles: Set[Obstacle]):
        ...
        # Que of (position, path), path = (position, path)
        queue = deque([(start, [])]) 
        ...
        while queue:
            ...
            for d in [Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT]:
                ...
                if  (
                        ...
                        next_pos not in [obs.position for obs in obstacles[0]] and
                        not next_pos.check_bounds(GRID_WIDTH, GRID_HEIGHT)
                    ):
                    self.visited.add(next_pos)
                    queue.append((next_pos, path + [Direction((dx, dy))]))

    def dfs(self, snake: Snake, food: Food, *obstacles: Set[Obstacle]):
        start = snake.get_head_position()
        goal = food.position
